...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv vinta_teste` or `python -m venv vinta_teste-venv`.
...
- [ ] Make sure the virtualenv is activated `workon vinta_teste` or `source vinta_teste-venv/bin/activate`.
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `vinta_teste/backend/vinta_teste/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `vinta_teste/backend/vinta_teste/settings/production.py`
...
- Create a copy of ``vinta_teste/settings/local.py.example``:  
  `cp vinta_teste/settings/local.py.example vinta_teste/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/vinta_teste/settings/base.py``.
- Adjust ``backend/vinta_teste/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
